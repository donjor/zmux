...
- **Shared agent doctrine registry and projections** `agents` `skills` `pi` — `agent-doctrine/` owns neutral terminal rules under `rules/shared/` and human-authored Markdown live scenarios under `scenarios/shared/`, deterministically generating the Claude runtime reference and the capability/caveat matrix, plus local Claude-harness worker prompts and host answer keys rendered to stdout on demand. The compact Pi guidance and package coverage manifest ship as frozen projections with the deferred Pi extension. `make check-doctrine`, CI, Worktrunk/pre-push gates, the single doctrine doctor, and live `./dev.sh zmux` reject stale or incomplete committed runtime projections without regenerating them.
- **Canonical agent-driven usage test framework** `agents` `qa` — the durable handwritten Claude flow lives under `agent-doctrine/harnesses/claude/`, drives one ordinary visible worker across 20 shared scenario prompts, and requires real state evidence plus exact cleanup on a user-confirmed native-or-edge lane. Maintainer-only prompt and answer-key projections render to stdout on demand instead of shipping tracked copies inside the skill. The parallel Pi harness is deferred with the Pi extension reintegration.
...
- **Instrumented peer handoffs resume Pi from lifecycle completion** `agents` `pi` — atomic `peer_handoff` now arms a fresh `turn:ready` wait, marks the peer running, and only then submits; running peers silently receive a replacement lifecycle wait when a window expires, and only a terminal outcome is delivered as a triggering follow-up by default. Output/idle are explicit fallbacks, `nextTurn` plus `triggerTurn` is rejected, the dispatcher now uses the canonical callback registry instead of a promoted duplicate, and the agent-driven usage test matrix covers low-tier Pi, Claude, Codex, and Agy plus shell-command lifecycle.
