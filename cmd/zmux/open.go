...
to take over instead.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) >= 2 {
			...
			targetSession = resolveLastActive(ws)
		}
		...
		return attachSession(targetSession)
	},
}
...
func resolveLastActive(ws *workspace.Workspace) string {
...
func init() {
	openCmd.Flags().BoolVar(&openHijackFlag, "hijack", false, "take over session from other client")
