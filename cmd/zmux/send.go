...
// Alias for common pattern: send text + Enter
...
func init() {
	sendCmd.Flags().StringVarP(&sendSessionFlag, "session", "s", "", "target session (default: current)")
	typeCmd.Flags().StringVarP(&sendSessionFlag, "session", "s", "", "target session (default: current)")
