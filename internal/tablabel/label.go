...
// cwd suffix like "[zmux]" when zmux has set @zmux_duplicate_name — which it does
// only when the window's name is duplicated in the session AND its cwd basename
// is unique among those duplicates, so the suffix actually disambiguates rather
// than echoing an identical string on both tabs (the marker is set by
// refreshDuplicateWindowNameMarkersForSession). With a zmux label, it renders
// "label [auto]" where the bracketed auto-name is dimmed; manual labels are
// already the disambiguator.
