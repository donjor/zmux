...
func commandSections() []Section {
	return []Section{
		...
		{Title: "Workspace Management", Entries: []Entry{
			...
			{"zmux tab inspect <tab>", "Inspect status + output + warnings"},
			{"zmux tab peer ensure <tab>", "Create/reuse peer + readiness evidence"},
		...
		{Title: "Terminal Commands", Entries: []Entry{
			...
			{"zmux wait <tab> --for <cond>", "Structured condition wait (turn/cmd/output/idle)"},
			...
			{"zmux type <tab> '<text>'", "Type text + Enter (+ optional wait)"},
