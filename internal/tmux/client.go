...
func (c *Client) ListSessions() ([]Session, error) {
	out, err := c.run("list-sessions", "-F",
		"#{session_name}\t#{session_windows}\t#{session_attached}\t#{session_activity}\t#{session_path}\t#{session_created}\t#{session_last_attached}\t#{session_group}\t#{@zmux_managed}\t#{@zmux_workspace}\t#{@zmux_session_label}\t#{@zmux_session_id}\t#{@zmux_clone}")
	if err != nil {
		return nil, err
	}
