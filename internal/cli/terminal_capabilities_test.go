...
func TestRunTerminalRefreshCurrentClient(t *testing.T) {
	...
	cmd := newTerminalRefreshCmd(a)
	var out bytes.Buffer
	cmd.SetOut(&out)
	if err := runTerminalRefresh(a, cmd, &terminalRefreshFlags{}); err != nil {
...
func TestRunTerminalRefreshExplicitTargetOutsideTmux(t *testing.T) {
	...
	cmd := newTerminalRefreshCmd(a)
	if err := runTerminalRefresh(a, cmd, &terminalRefreshFlags{targetClient: "/dev/pts/13", session: "pi"}); err != nil {
...
func TestRunTerminalCapabilitiesJSONOK(t *testing.T) {
	...
	cmd := newTerminalCapabilitiesCmd(a)
	var out bytes.Buffer
	cmd.SetOut(&out)
	if err := runTerminalCapabilities(a, cmd, &terminalCapabilitiesFlags{json: true}); err != nil {
...
func TestRunTerminalCapabilitiesTextMissingRGB(t *testing.T) {
	...
	var out bytes.Buffer
	cmd.SetOut(&out)
	if err := runTerminalCapabilities(a, cmd, &terminalCapabilitiesFlags{}); err != nil {
		t.Fatalf("runTerminalCapabilities failed: %v", err)
	}
	text := out.String()
