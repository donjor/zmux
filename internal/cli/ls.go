...
func newLsCmd(app *apppkg.App) *cobra.Command {
  ...
  zmux ls -s           List all sessions (flat, legacy mode)`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				return lsWorkspaceSessions(app, args[0])
			}

			return lsWorkspaces(app)
		},
	}
	cmd.Flags().BoolVarP(&lsSessionsFlag, "sessions", "s", false, "list all sessions (flat)")
	return cmd
}

func lsWorkspaces(app *apppkg.App) error {
	// Reconcile first.
	...
	for _, ws := range workspaces {
		...
		dir := shortenPathCLI(app, ws.RootDir)
...
func shortenPathCLI(app *apppkg.App, path string) string {
