...
func TestCreateFromTemplate(t *testing.T) {
	...
	tmpl := Template{
		...
		Options: TemplateOptions{Focus: "editor"},
	}

	err := CreateFromTemplate(m, tmpl, "myproject", "/home/user/project")
	if err != nil {
	...
	// Should have called NewWindow for the 2nd and 3rd windows.
	newWindowCount := 0
	for _, c := range m.Calls {
	...
	if newWindowCount != 2 {
		t.Errorf("expected 2 NewWindow calls, got %d", newWindowCount)
	}

	// Should have called SendKeys for editor (nvim .) and git (git status).
	sendKeysCount := 0
	for _, c := range m.Calls {
	...
	if sendKeysCount != 2 {
		t.Errorf("expected 2 SendKeys calls, got %d", sendKeysCount)
	}

	// Should have called SelectWindow for focus.
	selectCount := 0
	for _, c := range m.Calls {
		if c.Method == "SelectWindow" {
			selectCount++
		}
	}
	if selectCount != 1 {
