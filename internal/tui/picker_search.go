...
func matchWorkspaces(query string, workspaces []WorkspaceViewModel) []WorkspaceViewModel {
	if query == "" {
		// Clear any stale MatchedIndexes.
		result := make([]WorkspaceViewModel, len(workspaces))
	...
	result := make([]WorkspaceViewModel, len(matches))
