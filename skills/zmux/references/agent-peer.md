...
CLI in a zmux tab; deliver prompts with `zmux type` / Pi dispatcher `type_text` so the user
...
In Pi, prefer dispatcher `tab_inspect` for the status+output bundle and
