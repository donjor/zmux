...
# ── Link binaries ──
mkdir -p "$BIN_DIR"
ln -sf "$ZMUX_ROOT/bin/zmux" "$BIN_DIR/zmux"
ln -sf "$ZMUX_ROOT/bin/zmux-apply-theme" "$BIN_DIR/zmux-apply-theme"
printf "  ${green}✓${reset} linked zmux → %s/zmux\n" "$BIN_DIR"
...
printf "\n"
printf "  ${gold}${bold}run: zmux init${reset}\n\n"
