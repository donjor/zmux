...
Render `claude-prompts`, send its session contract once, then send one rendered scenario prompt per checkpoint. Render `claude-answer-key` separately and keep that output host-side; never quote its expected verbs to the worker.
...
Use the `claude-answer-key` render output host-side. A passing worker may use equivalent documented Claude CLI composition, but must preserve the neutral outcome and evidence.
