...
`checklists/natural-shell-lifecycle.toml` verifies root-shell lifecycle behavior
on `zzmux`: `bash --norc` and foreground TTY children survive,
raw typed commands publish running/done metadata, persistent agent/TUI venues do
not leave a stuck spinner, `zmux run` no longer prints completion sentinels,
agent CLI version/help smokes do not wedge, and failed commands report the
