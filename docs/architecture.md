...
| Package     | Lines | Role                                                                                                                                                                                                                                                                                                                  |
| ----------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config`    | ~375  | TOML config loading, defaults, `FS` interface for tests                                                                                                                                                                                                                                                               |
| `debug`     | ~95   | Opt-in debug logging (`ZMUX_DEBUG=1`)                                                                                                                                                                                                                                                                                 |
| `procfs`    | ~65   | Linux `/proc` process-tree inspection                                                                                                                                                                                                                                                                                 |
| `tablabel`  | ~35   | Stable optional tab-label overlay format                                                                                                                                                                                                                                                                              |
| `termtitle` | ~80   | tmux terminal-title format contract + parser (no zmux deps; dissolves the latent tmux↔terminal cycle)                                                                                                                                                                                                                 |
| `terminal`  | ~260  | Resolves strict screenshot targets for the current tmux client                                                                                                                                                                                                                                                        |
| `keys`      | ~440  | Keybinding registry — single source of truth for `conf.go`, help surfaces, and the generated `docs/reference/keybindings.md`                                                                                                                                                                                                    |
| `guard`     | ~500  | Classifies a shell command for agent terminal-hygiene (raw-tmux / background-job / shared-interaction), recursively scanning nested-form payloads (`sh -c`, env/path-prefixed shells, `xargs`, here-docs); ruleset shares `testdata/zmux-guard-corpus.jsonl` with the Claude hook + pi classifier. Pure leaf, no deps |

### tmux boundary

| Package | Lines | Role                                                                                                                                                 |
| ------- | ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tmux`  | ~2900 | Typed wrapper around the `tmux` CLI — `Runner` interface, `MockRunner` for tests, generated `tmux.conf`, logical-pane scan, and placement primitives |
...
| Package      | Lines | Role                                                                                                                                                                                                                                                                                                              |
| ------------ | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `session`    | ~630  | Session model, CRUD, tmp-session helpers                                                                                                                                                                                                                                                                          |
| `recipe`     | ~1650 | Recipe discovery, TOML parsing, pure planning, dry-run rendering, execution, plus fork/extend/create authoring                                                                                                                                                                                                    |
| `workspace`  | ~1450 | Workspace state (TOML), v3 local-label session identity, session tracking, reconciliation                                                                                                                                                                                                                         |
| `tabs`       | ~2100 | Logical-tab identity, scanning, placement, dock hide/show, MRU, reconciliation, lifecycle stamps (`lifecycle.go`), display-state resolution, and the reaper (`reap.go`)                                                                                                                                            |
| `tabstate`   | ~300  | Pane-canonical human lifecycle state (`attention` / `failed` / `running` / `ready` / `done`) and bar glyph formatting                                                                                                                                                                                             |
| `theme`      | ~580  | Theme parsing, semantic palette, resolver, embed of bundled themes                                                                                                                                                                                                                                                |
| `bar`        | ~2870 | Status bar generation, presets, two-line logical-tabs rendering, preview                                                                                                                                                                                                                                          |
...
| `overmind`   | ~70   | Overmind control client (`Client` interface: connect/restart/stop/logs)                                                                                                                                                                                                                                           |
| `setup`      | ~540  | Shell-rc integration: pure plan + apply behind `config.FS` (markers, `.bak`, dry-run, removal)                                                                                                                                                                                                                    |
...
| `waitfor`    | ~590  | First-class condition waiting for agent flows: fresh command lifecycle, fresh peer turn lifecycle, output-regex baseline waits, and idle/quiet fallback over `tmux.Runner`                                                                                                                                        |
| `runexec`    | ~270  | Post-delivery mechanics of `zmux run`: temp command-script writing, blocking on the shell-lifecycle `@zmux_run_result`, `--until` readiness, and live output following. Cobra/target/tab-policy stay in `internal/cli`; this takes a `tmux.Runner`, injected `io.Writer`s, and a cancellable `context` so the interrupt/dedupe/follow loops are deterministically testable |
| `qa`         | ~1320 | Repo-local QA walkthrough framework: checklist parse/lint, scorecard state, shell runner, and `cmd/qa` CLI                                                                                                                                                                                                        |
| `actions`    | ~150  | Typed action model (payloads) the command palette executes through `tmux.Runner`                                                                                                                                                                                                                                  |
| `help`       | ~190  | Help-content SSOT (command + keybinding reference) shared by `zmux help` and the prefix+? viewer, so the two can't drift                                                                                                                                                                                          |
...
The render pipeline reads from `time.Now()` for timestamps, `bar.Prober` for git branch/dirty/ahead-behind and language detection, `tmux.Runner` for session/window metadata, and the resolved palette from `internal/theme`. Git/language side-effects are behind the `Prober` seam; the bar's timestamp reads `time.Now()` directly. The bar is not the only non-injected clock owner, though — the `internal/workspace` and `internal/session` stores also call `time.Now()` directly for durable timestamps and IDs. A shared Clock seam covering those stores is deferred: current tests don't need it, so none is added just to satisfy this doc. Shared tmux options in `generate.go` also own pane-border headers: single-pane and split windows use the same `<index> <name> <detail>` format, while `status-right` stays reserved for volatile status segments.
